<!--
  file: ch02_conditional
  tier: A
  outcomes: 1c,1e,1f,1g
  tia: A.2
  locale: Cerulean
  maps_to: Viridian -> Cerulean, Misty -- reasoning from evidence (the proving ground)
-->

# Reasoning From Evidence {.type-water}

<figure>
<img src="../../assets/maps/kanto_ch02.png" alt="Kanto town map with the road from Viridian City north to Cerulean City highlighted; Cerulean, home of the Cascade Badge, is marked as the current destination." style="width:70%; max-width:520px; display:block; margin:1em auto;">
...
You walk into Cerulean to chaos. The bridge into town is cordoned with tape, Officer Jenny is pacing, and a rumor races down the street: a *phantom thief* has been slipping into the Pokémon Center at night and emptying trainers' item bags. Last night it struck again — and for a tense minute, because you are the unfamiliar face who just arrived, Jenny's eyes settle on *you*.

"We have a witness," Jenny says, frowning at her notepad. "Whoever it was had a **Water-type Pokémon** with them — there were wet footprints all the way to the door." She looks past you, relaxing slightly. "Problem is, half the trainers in this town carry Water-types. Cerulean's practically the water capital of Kanto."
...
3. If $P(A) = 0.3$, what is $P(A^c)$? *(Answer: $1 - 0.3 = 0.7$.)*
...
- **Recognize and apply independence** — pairwise versus mutual — without confusing it with "mutually exclusive," and use the **addition rule** for unions. *(Outcome 1e.)*
...
> *Exam-weight signpost.* Conditional probability, independence, and Bayes together are among the **most heavily tested** ideas in Exam P's General Probability section (23–30% of the exam). This is a **Tier A** chapter: it earns the full treatment, and what you build here is reused in every chapter that follows.
...
**Beat 2 — Anchor + concrete instance.** Remember the Venn rectangle from Route 1: all of $S$. Conditioning *crops* that rectangle. Here is the story, with real numbers.
...
**Beat 8 — Picture it.** Picture the "shrink" literally: the full town of 60, with the 24 Water-type owners highlighted; the moment you condition, that block of 24 *becomes* the entire frame, and the 9 locals go from being $9/60$ of the town to $9/24$ of what remains. Re-cropping to $B$ **is** dividing by $P(B)$.
...
Brock — a young rock-Pokémon trainer you'll meet down the road — once left his ball bag in your pack: **7 Poké Balls and 3 Great Balls**, jumbled. You'll draw **two in the dark, without replacement**. You need both to be Poké Balls. *What is $P(\text{both Poké})$?*
...
<figure>
<img src="../../assets/diagrams/ch02_tree_sequential.png" alt="A two-stage probability tree for a bag of 7 Poke Balls and 3 Great Balls drawn without replacement. The root splits into Poke (7/10) and Great (3/10). The Poke branch splits into Poke (6/9) and Great (3/9); the Great branch splits into Poke (7/9) and Great (2/9). Each of the four leaves is labeled with its multiplied path probability: 42/90, 21/90, 21/90, 6/90, and the four leaves sum to 90/90 = 1." style="width:78%; max-width:560px; display:block; margin:1em auto;">
...
On **Route 24** — the river path just north of Cerulean — you log **100 Pokémon**: **40 Water-type**, **30 live in tall grass**, **12 are both**. *What fraction is Water-type or grass-dwelling (at least one)?*
...
Then $P(A\cap B)=0$ and the subtraction disappears. (Recall the **complement** $A^c$ — "not $A$" — from Route 1: since $A$ either happens or it doesn't, $P(A) + P(A^c) = 1$. That single fact powers every "$1 - P(\text{none})$" shortcut you'll use next.)
...
**Beat 8 — Picture it.** Two overlapping circles over the 100 Route-24 logs: the Water circle holds 40, the grass circle holds 30, and the lens where they overlap holds 12. The shaded lens is the piece you counted in *both* piles, so it must come back out exactly once. When two circles don't touch at all (the disjoint case), there is no lens — nothing to subtract.
...
**Bill's lighthouse**, up the coast from Cerulean, runs on **two independent circuits**. Circuit 1 works with probability **$0.9$**, circuit 2 with probability **$0.8$**. They don't talk to each other. The beacon **shines** if at least one works. *Find $P(\text{both work})$ and $P(\text{beacon shines})$.*
...
- *Twist (pairwise $\neq$ mutual):* you can have three events that multiply two-at-a-time yet **fail** to multiply all-three-at-once. Take a spinner with **four equally likely sectors** $\{1,2,3,4\}$ and let $A=\{1,2\}$, $B=\{1,3\}$, $C=\{1,4\}$: each has probability $\tfrac24=\tfrac12$, every pair meets in the single sector $\{1\}$ so $P(A\cap B)=\tfrac14=\tfrac12\cdot\tfrac12$ (pairwise independent!), but the triple also meets only in $\{1\}$, giving $P(A\cap B\cap C)=\tfrac14 \neq \tfrac18 = P(A)P(B)P(C)$. Knowing two of them forced the third (all three share sector 1) — so the three together are *dependent*. Pairwise independence is genuinely weaker. *(This is Problem C2.18.)*
- *Edge:* "at least one of $n$ independent events" is always $1 - \prod_i (1-p_i)$, the complement of "all fail."

**Beat 8 — Picture it.** Think of a 2×2 area grid: the column width is $P(A)$ and the row height is $P(B)$. Under **independence**, the joint cell's area is exactly width × height — the product of the margins. When the events are **dependent**, the realized $P(A\cap B)$ overshoots or undershoots that rectangle. Independence is the visual statement "the joint cell equals the product of the margins."
...
- *Two-case form:* with just $B$ and its complement $B^c$ ("not $B$," from Concept 3), the law reads $P(A) = P(B)P(A\given B) + P(B^c)P(A\given B^c)$ — the "rainy day / dry day flood" pattern (Problem C2.5).
...
<figure>
<img src="../../assets/diagrams/ch02_total_prob.png" alt="A one-stage fan diagram. A root node labeled 'lead' branches to three leaves: Staryu (prior 0.5), Starmie (0.3), Psyduck (0.2). Each leaf carries its super-effective rate (0.3, 0.6, 0.1) and the product on its branch: 0.15, 0.18, 0.02. The three products are summed at the bottom to 0.35." style="width:78%; max-width:560px; display:block; margin:1em auto;">
...
<img src="../../assets/sprites/front/120.png" alt="Staryu, a Water-type Pokémon" style="width:130px; display:block; margin:0 auto; image-rendering: pixelated;">
...
- *$n$-way:* more than two causes — three breeding pools, three suspect groups — just add more rows; the denominator sums them all (Worked Example 2; the Gym Battle).
- *Twist (negative evidence):* Bayes works for a *non*-event too. "Scanned **negative**" just uses $P(-\given D) = 1-0.95$ and $P(-\given D^c) = 1-0.08$ in the same grid (Problem C2.6).
- *Edge (zero likelihood):* if a cause makes the evidence *impossible* ($P(A\given B_k)=0$), its posterior is $0$ — that cause is eliminated (Problem C2.15).
- *Twist (compound likelihood — chained conditions):* so far each cause produced the evidence in **one** step. But a likelihood can itself be built from *chained* conditions, multiplied together (multiplication rule, Entry №02). Suppose to be the culprit a suspect must **first choose to act**, and **then leave a clue** — two conditional factors, not one. If a suspect acts w.p. $P(\text{act}\given B_k)=0.05$ and, *having acted*, leaves a clue w.p. $P(\text{clue}\given\text{act}\cap B_k)=0.40$, then the likelihood that this suspect both acted *and* left the clue is the product
$$P(\text{act}\cap\text{clue}\given B_k) = P(\text{act}\given B_k)\,P(\text{clue}\given\text{act}\cap B_k) = 0.05\times 0.40 = 0.02.$$
That single $0.02$ then drops into the likelihood column of the grid exactly like any other — Bayes does not care that you *built* it from two factors. The Gym Battle below extends this to **three** chained factors (belong to a group $\times$ steal $\times$ leave a clue).
...
<figure>
<img src="../../assets/diagrams/ch02_bayes_grid.png" alt="The detective's grid: a five-column table (Cause, Prior, Likelihood, Joint = product, Posterior = joint over column total) filled with the Sick-Staryu numbers. Sick row: prior 0.02, likelihood 0.95, joint 0.0190, posterior 0.195. Healthy row: prior 0.98, likelihood 0.08, joint 0.0784, posterior 0.805. Arrows show multiply-across to get the joint, then sum the joint column to P(+) = 0.0974, then divide each joint by 0.0974 to get the posteriors, which sum to 1." style="width:82%; max-width:600px; display:block; margin:1em auto;">
<figcaption>The Bayes table for the Sick Staryu. Multiply across $\to$ sum the joint column ($P(+)=0.0974$) $\to$ divide. Posteriors $0.195 + 0.805 = 1$.</figcaption>
</figure>
...
<figure style="margin:1.5em auto; max-width:160px; text-align:center;">
<img src="../../assets/characters/misty.png" alt="Misty, the Cerulean Gym Leader" style="width:140px; display:block; margin:0 auto; image-rendering: pixelated;">
...
### Worked Example 1 — The Sick Staryu (full narration; understanding-first)
...
**Step 3 — Trainer's Path (the fast how: grid it).** Same numbers, no prose. On the TI-30XS MultiView, evaluate the *whole* denominator first, then store that single number so you don't retype it — [ ( 0.02 [×]{.kbd} 0.95 ) [+]{.kbd} ( 0.98 [×]{.kbd} 0.08 ) [STO▸]{.kbd} [x]{.kbd} [enter]{.kbd} ]{.keystroke} evaluates $P(+)=0.0974$ and banks it in memory variable $x$ ($\texttt{STO▸}$ stores the *running result*, so the expression must be complete before you press it). Then divide the sick joint by the recalled total — [ 0.0190 [÷]{.kbd} [x]{.kbd} [enter]{.kbd} ]{.keystroke} — for the posterior $\approx 0.195$.
...
### Worked Example 2 — Which Pool Did the Magikarp Come From? (partial guidance)
...
<img src="../../assets/sprites/front/129.png" alt="Magikarp, a Water-type Pokémon" style="width:110px; display:block; margin:0 auto; image-rendering: pixelated;">
<figcaption style="font-size:0.8em;">#129 Magikarp</figcaption>
</figure>
<figure style="margin:0; text-align:center;">
<img src="../../assets/sprites/front/130.png" alt="Gyarados, Magikarp's evolved form" style="width:110px; display:block; margin:0 auto; image-rendering: pixelated;">
...
### Worked Example 3 — Two Poké Balls, No Replacement (light guidance)
...
### Worked Example 4 — Misty's Hidden Move (exam speed)

**ARCHETYPE:** *Law of total probability (forward; no inversion).*

<figure style="margin:1.25em auto; max-width:130px; text-align:center;">
<img src="../../assets/sprites/front/54.png" alt="Psyduck, a Water-type Pokémon" style="width:110px; display:block; margin:0 auto; image-rendering: pixelated;">
...
**Check & pitfall.** $0.35$ lies between the smallest rate $0.1$ and the largest $0.6$ — every weighted average must ✓. **Pitfall:** the plain average $\tfrac{0.3+0.6+0.1}{3}=0.333$, which ignores the lead weights. *(If instead you're told it **was** super-effective and asked which lead — that's a Bayes inversion, Problem C2.13.) (Back-ref: Entry №05.)*

## Trainer's Tips

::: trainers-tip
**TRAINER'S TIP — Always grid the Bayes (and store the denominator)**

Under exam time, don't juggle the Bayes fraction in your head. Draw the four-column grid (prior · likelihood · joint · posterior), fill it left to right, and the **sum of the joint column is your denominator automatically**. On the TI-30XS MultiView, the killer move is **store-don't-retype**: compute the joint-column total once and bank it with [STO▸]{.kbd} into a memory variable, e.g. [ ( 0.02 [×]{.kbd} 0.95 ) [+]{.kbd} ( 0.98 [×]{.kbd} 0.08 ) [STO▸]{.kbd} [x]{.kbd} ]{.keystroke}. Then every posterior is just [ joint [÷]{.kbd} [x]{.kbd} [enter]{.kbd} ]{.keystroke} — no rounding drift, no retyping $0.0974$ three times. The posteriors must add to $1$; if they don't, you mis-multiplied.
:::
...
When you read "at least one" with independent trials, compute $1 - P(\text{none}) = 1 - \prod_i (1-p_i)$ rather than summing a pile of cases. On the TI-30XS MultiView, chain the $(1-p_i)$ products, then subtract from $1$ in one shot — faster, with fewer sign slips. For two events with $p_1=0.3,\,p_2=0.4$: [ 1 [−]{.kbd} ( 1 [−]{.kbd} 0.3 ) ( 1 [−]{.kbd} 0.4 ) [enter]{.kbd} ]{.keystroke}.
:::
...
<figure>
<img src="../../assets/diagrams/ch02_false_positive.png" alt="An area diagram of 10,000 Water Pokemon split into a narrow Sick block (200) and a wide Healthy block (9,800). In the Sick block, 190 are shaded as true positives. In the Healthy block, a thin 8% band of 784 is shaded as false positives. The caption shows that among the 974 positives, only 190 are truly sick, giving P(sick | positive) = 190/974 = 0.195." style="width:78%; max-width:560px; display:block; margin:1em auto;">
<figcaption>Base rate beats accuracy: even a $95\%$-sensitive test on a rare ($2\%$) condition is mostly false alarms — the $784$ false positives bury the $190$ true ones.</figcaption>
</figure>
...
*(A second consistent night would square each likelihood and drive the Rocket posterior to $\approx 0.94$ — see Problem C2.20. Evidence compounds.)*

## The Gym Challenge — Problem Set

::: problem-set
**THE CERULEAN CASE FILE — your questline.** Officer Jenny has deputized you. This problem set is one escalating mission: the **Route Trainer** legs are field-work around Cerulean (cataloguing, scouting, quick scans); the **Gym Battle** tier is the boss fight (the phantom-thief case at exam difficulty); the **Elite Challenge** tier is optional post-game once the badge is yours. Work it timed (~6 min/problem), then check the **Answer Key** below. Hit the mastery bar (**80%+ with correct method**) to clear the questline and claim the Cascade Badge. Problems are listed first; full worked solutions follow afterward (never interleaved). Markers: 🔴 routine method · 🟡 routine-with-a-twist · 🔵 stretch.

### Route Trainers (the early legs — scouting & cataloguing)

**C2.1.** 🔴 *(Jenny's first task: catalog the wild Pokémon near the crime scene.)* On Route 24 you log $100$ Pokémon: $40$ are Water-type, $30$ live in tall grass, $12$ are both. Given that one lives in tall grass, find the probability it is Water-type.

**C2.2.** 🔴 *(Inventorying the Center's recovered ball stash.)* Brock's bag has $5$ Poké Balls and $4$ Great Balls. Draw two without replacement. Given the first is a Poké Ball, find the probability the second is a Great Ball.

**C2.3.** 🔴 *(Checking whether the Center's alarm beacon could have fired.)* Bill's lighthouse beacon runs on two independent circuits: circuit 1 works w.p. $0.9$, circuit 2 w.p. $0.8$. The beacon shines if at least one works. Find $P(\text{shines})$.

**C2.4.** 🟡 *(Misty and Ash each try to net a witness Pokémon.)* Ash and Misty independently try to catch a Tentacool: Ash succeeds w.p. $0.6$, Misty w.p. $0.7$. Find the probability that exactly one of them catches it.

**C2.5.** 🟡 *(Was last night a flooding night? It matters for the footprints.)* $30\%$ of Cerulean days are rainy. The gym floods w.p. $0.5$ on rainy days and $0.05$ on dry days. Find the probability the gym floods on a random day.

**C2.6.** 🟡 *(A suspect's Pokémon scans NEGATIVE for Brineflux — does that clear it?)* Nurse Joy's quick-scan flags $90\%$ of genuinely sick Pokémon and $20\%$ of healthy ones; $10\%$ of those scanned are sick. A Pokémon scans **negative**. Find $P(\text{sick}\given\text{negative})$.

**C2.7.** 🔴 *(Two TMs were taken from the recovered stash.)* A stack holds $3$ Fire TMs and $2$ Water TMs. Draw two in order without replacement. Find the probability both are Fire TMs.

**C2.8.** 🟡 *(AUDIT — Team Rocket's recon notes claim two traits are independent.)* Your tallies of wild Pokémon give $P(A)=0.4$ for "Water-type," $P(B)=0.5$ for "found near the river," and $P(A\cup B)=0.65$. Meowth's note declares "$A$ and $B$ are independent — assume it." Are they? Compute $P(A\cap B)$ and the independence check, and name Meowth's error if he is wrong.

**C2.22.** 🔴 *(DECISION — one ball each at two different wild Pokémon, throws independent.)* You throw a Poké Ball at one wild Pokémon (catches w.p. $0.6$) and, separately, a Great Ball at another (catches w.p. $0.75$). The two throws are **independent**. Find the probability **both** catch and the probability **at least one** catches.

> *Questline beat: the legwork is done. Jenny hands you the case file. The Gym Battle is the boss.*

### Gym Battles (the boss fight — true SOA difficulty)

**C2.9.** 🟡 *(A witness Pokémon FLED — which species was it?)* A wild Pokémon is Goldeen ($50\%$), Seaking ($30\%$), or Horsea ($20\%$). Each flees if you hesitate: Goldeen $0.2$, Seaking $0.5$, Horsea $0.8$. The Pokémon **fled**. Find $P(\text{Horsea}\given\text{fled})$.

**C2.10.** 🟡 *(AUDIT — a "Voltorb" decoy was planted at the scene; it did NOT explode.)* A disguised Voltorb explodes w.p. $0.7$ if it's a live decoy and $0.05$ if it's a dud; the bag is $20\%$ live decoys. You toss one and it **does not explode**. Team Rocket's planted note says "no explosion means it was almost certainly a dud — about $95\%$." Find the true $P(\text{live}\given\text{no explosion})$ and name the error.

**C2.11.** 🟡 *(How many Shiny eggs are in the stolen incubator?)* A box has $10$ eggs, $3$ Shiny. Draw two without replacement. Find the probability **at least one** is Shiny.

**C2.12.** 🟡 *(Three Game-Corner machines were tampered with; which paid out?)* Three machines pay out independently: $X$ w.p. $0.2$, $Y$ w.p. $0.3$, $Z$ w.p. $0.25$. One pull each. Find the probability **exactly two** pay out.

**C2.13.** 🔵 *(RIVAL TRAP — inverts WE 4. Gary watches Misty's opener and brags.)* Misty leads Staryu ($0.5$), Starmie ($0.3$), Psyduck ($0.2$) with super-effective rates $0.3, 0.6, 0.1$. Her opener **was** super-effective. Gary says "obviously it was Starmie — that's the strongest, has to be at least $60\%$." Find the true $P(\text{Starmie}\given\text{super-effective})$ and say whether Gary is right.

**C2.14.** 🔵 *(A rare Pokérus screen on a suspect's Pokémon comes back positive.)* A diagnostic for rare Pokérus has sensitivity $0.99$ and specificity $0.96$; prevalence $0.5\%$. A Pokémon tests positive. Find $P(\text{infected}\given\text{positive})$ and comment on why it is small.

**C2.15.** 🟡 *(One of the recovered balls is a trick ball; a thrown ball failed to release.)* Of three identical-looking Poké Balls, one is a trick ball (releases its Pokémon $80\%$ of the time), the other two release $100\%$ of the time. You pick one at random, throw it, and the **Pokémon stays in**. Find $P(\text{trick}\given\text{stays in})$.

**C2.16.** 🔵 *(Reading a battle that the witness only half-saw.)* Let $A$ = "the lead was Electric" and $B$ = "the trainer won the opening turn." The lead is Electric $30\%$ of the time ($P(A)=0.3$); when it is, they win turn one w.p. $P(B\given A)=0.7$, and otherwise w.p. $P(B\given A^c)=0.4$. Find $P(A\given B)$ (won — was it Electric?) and $P(A\given B^c)$ (lost — was it Electric?).

**C2.17.** 🟡 *(Three lure urns at the scene; a red lure was found.)* Urn I has $2$ red, $3$ blue; Urn II has $4$ red, $1$ blue; Urn III has $1$ red, $4$ blue. Pick an urn uniformly, draw one lure; it's **red**. Find $P(\text{Urn II}\given\text{red})$.

**C2.18.** 🔵 *(A spinner-based alibi: is the evidence really independent?)* On a spinner, events $A,B,C$ each have probability $\tfrac12$ with all three pairwise intersections equal to $\tfrac14$, but $P(A\cap B\cap C)=\tfrac14$ *(constructed so that whenever any two of $A,B,C$ occur, the third does too — an intentional valid construction, not a typo, and a **second, different** valid construction from the four-sector spinner $A=\{1,2\},B=\{1,3\},C=\{1,4\}$ in Concept 4 Beat 7; don't expect the two to be the same object)*. Show $A,B,C$ are **pairwise independent but not mutually independent**.

> *Questline beat: the badge is yours. The post-game below is optional — but it's where the real detectives play.*

### Elite Challenge (post-game — integrative / stretch)

**C2.19.** 🔵 *(Counting + Bayes — which bag did the recovered pair come from?)* Bag 1 has $6$ Poké Balls and $4$ Great Balls; Bag 2 has $3$ Poké Balls and $7$ Great Balls. Pick a bag at random, draw **two without replacement**; both are Poké Balls. Find $P(\text{Bag 1}\given\text{two Poké})$.

**C2.20.** 🔵 *(Sequential updating — the thief strikes a SECOND night.)* The phantom thief strikes **two** nights running, each leaving wet footprints, by the same unknown group. Using the Gym Battle figures (Traveler share $0.60$, steal $0.05$, clue $0.40$; Local $0.35$, $0.01$, $0.20$; Rocket $0.05$, $0.30$, $0.90$), and treating the two nights as conditionally independent given the group, find $P(\text{Rocket}\given\text{two clue thefts})$. *(Restated given: the one-night posteriors were Traveler $0.458$, Local $0.027$, Rocket $0.515$.)*

**C2.21.** 🔵 *(DECISION — Bayes feeding Joy's quarantine rule.)* Joy quarantines only if the posterior of disease exceeds $0.5$. Prevalence $4\%$, sensitivity $0.9$, false-positive rate $0.05$. (a) After one positive scan, does she quarantine? (b) After **two independent** positive scans? Show the posterior in each case — the two options (quarantine now vs. wait for a second scan) are the same Bayes object, updated.
:::
...
**C2.1** — *(standard) Direct conditional probability (Entry №01).*
$$P(\text{Water}\given\text{grass}) = \frac{P(\text{Water}\cap\text{grass})}{P(\text{grass})} = \frac{12/100}{30/100} = \frac{12}{30} = 0.40.$$

**C2.2** — *(standard) Conditional probability, without replacement (Entry №02).* After removing one Poké Ball, $4$ Great Balls remain among $8$:
$$P(\text{2nd Great}\given\text{1st Poké}) = \frac{4}{8} = 0.50.$$

**C2.3** — *(standard) "At least one" via complement, independence (Entries №03, №04).*
$$P(\text{shines}) = 1 - P(\text{both fail}) = 1 - (0.1)(0.2) = 1 - 0.02 = 0.98.$$

**C2.4** — *(standard) Exactly-one of two independent events (Entry №04).*
$$P(\text{exactly one}) = 0.6(1-0.7) + (1-0.6)(0.7) = 0.18 + 0.28 = 0.46.$$

**C2.5** — *(standard) Law of total probability, forward (Entry №05).*
$$P(\text{flood}) = (0.3)(0.5) + (0.7)(0.05) = 0.15 + 0.035 = 0.185.$$

**C2.6** — *(standard) Two-way Bayes on a negative result (Entry №06).* Here $P(\text{neg}\given\text{sick})=1-0.90=0.10$ and $P(\text{neg}\given\text{healthy})=1-0.20=0.80$.
...
**C2.7** — *(standard) Chained multiplication, without replacement (Entry №02).*
$$P(\text{both Fire}) = \frac{3}{5}\cdot\frac{2}{4} = \frac{6}{20} = 0.30.$$

**C2.8** — *(audit) Independence test via the multiplication rule (Entries №03, №04).* By inclusion–exclusion, $P(A\cap B) = P(A)+P(B)-P(A\cup B) = 0.4+0.5-0.65 = 0.25$. Compare to $P(A)P(B) = (0.4)(0.5) = 0.20$. Since $0.25 \neq 0.20$, $A$ and $B$ are **not independent** — they are positively associated. **Meowth's error:** he *assumed* independence (you may never assume it; it must be told or derived). The true overlap $0.25$ exceeds the $0.20$ that independence would require.

**C2.9** — *(standard) $n$-way Bayes from a 3-way partition (Entry №06).*
...
**C2.10** — *(audit) Two-way Bayes on a non-event (Entry №06).* $P(\text{no boom}\given\text{live})=1-0.7=0.30$, $P(\text{no boom}\given\text{dud})=1-0.05=0.95$.
...
The planted note ("$95\%$ a dud") quotes $P(\text{dud}\given\text{no boom})=0.760/0.820\approx 0.927$, which *is* high — but the **error** is that this says nothing damning: the question is whether it was *live*, and the true $P(\text{live}\given\text{no boom})\approx 0.073$. (Team Rocket inflated "$\approx 0.93$" to "$95\%$" and pointed it at the wrong event.)

**C2.11** — *(standard) "At least one," without replacement, complement (Entries №02, №03).*
$$P(\text{no Shiny}) = \frac{7}{10}\cdot\frac{6}{9} = \frac{42}{90} = \frac{7}{15}, \qquad P(\text{at least one}) = 1 - \frac{7}{15} = \frac{8}{15} \approx 0.533.$$

**C2.12** — *(standard) Exactly-two of three independent events (Entry №04).* Sum the three "two win, one loses" products with $p_X=0.2, p_Y=0.3, p_Z=0.25$:
$$\begin{aligned}
...
**C2.13** — *(rival_trap) Bayes inversion of a total-probability setup (Entries №05, №06).* From WE 4, $P(E)=0.35$; the Starmie joint is $(0.3)(0.6)=0.18$.
$$P(\text{Starmie}\given E) = \frac{0.18}{0.35} \approx 0.514.$$
**Gary is wrong.** He confused the *likelihood* $P(\text{super-eff.}\given\text{Starmie})=0.60$ with the *posterior* $P(\text{Starmie}\given\text{super-eff.})\approx 0.514$ — the bar flipped (prosecutor's fallacy). Starmie *is* the most likely lead given the hit, but at $\approx 51\%$, not $60\%$.

**C2.14** — *(standard) False-positive classic, extreme base rate (Entry №06).* $P(+\given\text{inf})=0.99$, $P(+\given\text{not})=1-0.96=0.04$, prevalence $0.005$.
...
**C2.15** — *(standard) Bayes with equal priors, zero-likelihood elimination (Entry №06).* Priors $\tfrac13$ each; $P(\text{stay}\given\text{trick})=0.20$, $P(\text{stay}\given\text{normal})=0$.
...
**C2.16** — *(standard) 2×2 Bayes, both conditions (Entries №05, №06).* First $P(B) = (0.3)(0.7)+(0.7)(0.4) = 0.21+0.28 = 0.49$.
...
**C2.17** — *(standard) Classic urn Bayes (Entry №06).* Priors $\tfrac13$ each; $P(\text{red}\given\text{I})=\tfrac25$, $\tfrac45$ for II, $\tfrac15$ for III.
...
**C2.18** — *(standard) Pairwise vs. mutual independence (Entry №04).* Each pair multiplies: $P(A\cap B)=\tfrac14 = (\tfrac12)(\tfrac12) = P(A)P(B)$, and likewise for $A\cap C$ and $B\cap C$ — so the events are **pairwise independent**. But mutual independence additionally requires
...
**C2.19** — *(standard) Bayes with a without-replacement likelihood (Entries №02, №06).* Likelihood of "two Poké Balls" from each bag:
...
**C2.20** — *(standard) Sequential Bayesian updating, conditionally independent evidence (Entries №02, №06).* Per-night likelihood = steal $\times$ clue: Traveler $0.05\cdot0.40 = 0.02$, Local $0.01\cdot0.20 = 0.002$, Rocket $0.30\cdot0.90 = 0.27$. Two conditionally independent nights $\to$ square each, then weight by population share:
...
**C2.21** — *(decision) False-positive Bayes feeding a decision rule, repeated evidence (Entry №06).* Prevalence $0.04$, sensitivity $0.9$, false-positive rate $0.05$.
...
One test is inconclusive; a second independent positive flips the decision — the same compounding-evidence lesson as C2.20.

**C2.22** — *(decision) Independence: product rule for two independent successes (Entry №04).* Independence lets the joint factor:
...
| # | Answer | | # | Answer |
|---|---|---|---|---|
| C2.1 | $0.40$ | | C2.12 | $0.140$ |
| C2.2 | $0.50$ | | C2.13 | $\approx 0.514$ (Gary wrong) |
| C2.3 | $0.98$ | | C2.14 | $\approx 0.1106$ |
| C2.4 | $0.46$ | | C2.15 | $1$ |
| C2.5 | $0.185$ | | C2.16 | $0.4286;\ 0.1765$ |
| C2.6 | $\approx 0.0137$ | | C2.17 | $4/7 \approx 0.571$ |
| C2.7 | $0.30$ | | C2.18 | pairwise yes, mutual no |
| C2.8 | not independent | | C2.19 | $5/6 \approx 0.833$ |
| C2.9 | $\approx 0.390$ | | C2.20 | $\approx 0.938$ |
| C2.10 | $\approx 0.0732$ | | C2.21 | (a) $0.4286$ no; (b) $0.9310$ yes |
| C2.11 | $8/15 \approx 0.533$ | | C2.22 | both $0.45$; at least one $0.90$ |
:::

## Badge Earned — the Cascade Badge

<figure style="text-align:center; margin:1.5em auto;">
<img src="../../assets/badges/cascade_badge.png" alt="The Cascade Badge, a blue teardrop-shaped gym badge" style="width:140px; display:block; margin:0 auto;">
<figcaption class="badge-caption"><strong>Cascade Badge earned!</strong> Rank up: Rookie → 1 badge.</figcaption>
</figure>

You handed Officer Jenny the honest posteriors, the real thief was found, and Misty pressed the Cascade Badge into your hand. **Rank: Rookie (1 badge).** One down, seven to go on the road to the Indigo Plateau.

**Mastery checklist — tick each before you move on (mapped 1-to-1 to the SOA outcomes):**

- ☐ **(1c)** I can compute $P(A\given B) = P(A\cap B)/P(B)$, chain it with the multiplication rule, and walk a without-replacement tree. *(Rematch: Concepts 1–2, WE 3.)*
- ☐ **(1e)** I can test **independence** with $P(A\cap B)=P(A)P(B)$, use the **addition rule** with its overlap term, handle "at least one" by the complement, and I never confuse *mutually exclusive* with *independent*. *(Rematch: Concepts 3–4, Problems C2.4, C2.8, C2.18, C2.22.)*
- ☐ **(1f)** I can assemble an unconditional probability with the **law of total probability** over a partition, and I sanity-bound it between the smallest and largest conditional rate. *(Rematch: Concept 5, WE 4, Problem C2.5.)*
- ☐ **(1g)** I can **invert** a conditional with **Bayes' theorem** (two-way and $n$-way) using the detective's grid, and I can explain why a strong clue need not imply guilt (base-rate neglect / prosecutor's fallacy). *(Rematch: Concept 6, WE 1–2, the Gym Battle, Problems C2.9–C2.21.)*

**Gym Rematch pointers.** Missed a Bayes inversion? Re-run Worked Example 1 and the Gym Battle, then redo C2.13 and C2.14. Tripped on independence vs. disjoint? Concept 4, Beat 4, then C2.18. Reached for a plain average? Concept 5, Beat 4, then C2.5.

> Next stop: the **S.S. Anne** at Vermilion, where the crowd is too big to track person-by-person and you'll learn to describe a whole population by its *distribution*. Pack the grid — you'll reuse conditioning there too.
