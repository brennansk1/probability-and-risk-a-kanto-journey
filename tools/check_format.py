...
REQUIRED_HEADINGS = [
    ...
    "Gym Battle", "Gym Challenge", "Answers", "Badge",
]
...
def audit_file(path: Path):
    ...
    if path.parent == CH_DIR and not is_stub and not is_orientation and not is_checkpoint and not is_finale:
        ...
        i_solutions = heading_pos("Answers")
        if i_problems == -1:
            errors.append(f"{rel}: no problems list (missing 'The Gym Challenge')")
        if i_solutions == -1:
            errors.append(f"{rel}: no solutions list (missing 'Answers')")
        if i_problems != -1 and i_solutions != -1 and i_solutions < i_problems:
            errors.append(f"{rel}: 'Answers' must come AFTER the problem set")
    ...
    if path.parent == CH_DIR and is_checkpoint and not is_stub:
        if heading_pos("Answers") == -1:
            errors.append(f"{rel}: checkpoint missing an 'Answers'")
